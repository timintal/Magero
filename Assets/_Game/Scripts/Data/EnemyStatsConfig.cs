...
{
    public class EnemyStatsConfig : IUnitDamageProvider, IUnitHealthProvider, IUnitMovementSpeedProvider
    {
        ...
        public float GetSpeed(UnitType t, int level)
        {
            ...
            if (enemyUpgrades.ContainsKey(key))
            {
                if (t == UnitType.Minion)
                {
                    return enemyUpgrades[key].MinionSpeed;
                }
                else if (t == UnitType.Brute)
                {
                    return enemyUpgrades[key].BruteSpeed;
                }
                else if (t == UnitType.Warrior)
                {
                    return enemyUpgrades[key].WarriorSpeed;
                }
            }
